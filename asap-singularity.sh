...
if [ "$SCRATCH" != "" ]; then
    singularity run -B \
    $ASAP:/asap:ro,$DATA:/data,$SCRATCH:/var/scratch,/etc/group:/etc/group:ro,/etc/passwd:/etc/passwd:ro \
    asap_latest.sif \
    $SKIP_CHAR $SKIP_COMP $DEBUG
else
    singularity run -B \
    $ASAP:/asap:ro,$DATA:/data,/etc/group:/etc/group:ro,/etc/passwd:/etc/passwd:ro \
    asap_latest.sif \
    $SKIP_CHAR $SKIP_COMP $DEBUG
fi
