...
CARD = "${ASAP_HOME}/share/card-3.0.7"
...
final Path taxPath = projectPath.resolve( "abr" )

// create local tmp directory
final Path tmpPath = Paths.get( '/', 'var', 'tmp', "tmp-${System.currentTimeMillis()}-${Math.round(Math.random()*1000)}" )
try { // create tmp dir
...
// Name of the conda environment that runs rgi
def condaEnvName = 'rgi'
...
// RGI needs localDB files in the tmp directory

def tmpDir = Paths.get(tmpPath.toString())
def localDBDir = tmpDir.resolve("localDB")
Files.createDirectories(localDBDir)
def sourceDir = Paths.get("/home/CARD/localDB")
// Copy files

Files.list(sourceDir).each { sourceFile ->
    if (Files.isRegularFile(sourceFile)) {
        def targetFile = localDBDir.resolve(sourceFile.fileName.toString())
        Files.copy(sourceFile, targetFile, StandardCopyOption.REPLACE_EXISTING)
        println "Copied ${sourceFile.fileName} to ${localDBDir.toAbsolutePath()}"
    }
}
...
ProcessBuilder pb = new ProcessBuilder(
    ...
    --input_sequence ${localGenomeSequencePath} \
    --output_file ${cardOutput} --exclude_nudge --local
    """
    )
